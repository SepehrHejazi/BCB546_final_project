...
L <- mean(dge$samples$lib.size) * 1e-3 #average library size in thousands
M <- median(dge$samples$lib.size) * 1e-3 #median library size in thousands
c(L, M)
summary(lcpm)
...
#Previous study had pre-processed the data using R/Bioconductor package xps, calculated the detection above background (DABG), and kept genes with a DABG P-value<0.01 in at least two samples per group
...
#The figure shows the expression distribution of samples for normalized data
library(RColorBrewer)
nsamples <- ncol(dge)
col <- brewer.pal(nsamples, "Paired")
par(mar = c(2, 5, 2, 10))
boxplot(logCPM, col=col, main="", xaxt="n", ann=FALSE, xlim=c(0.5, 10.5), ylim = c(0,19))
title(main="Normalized count-data for miRNAs of wt and AGAT-deficient mice", ylab = "logCPM")
...
topTreat <- topTreat(tfit, coef=1, n=Inf) #table of top genes from a linear model fit
topTable <- topTable(efit, coef=1 , n=Inf) #for results from eBayes
